        ...
        <ResultBox
          ...
          action={
            <SaveButton
              onClick={() =>
                onCommit(
                  `BMR ${sex} ${weight}kg ${height}cm ${age}y`,
                  `${Math.round(res.tdee)} kcal`,
                  "Health",
                )
              }
            />
...
export function BodyFatCalc({ onCommit }: { onCommit: CommitFn }) {
  ...
  return (
    <ToolCard title="Body Fat %" description="US Navy circumference method">
      ...
      {sex === "female" ? (
        <Field id="bf-hip" label="Hip" value={hip} onChange={setHip} suffix="cm" />
      ) : null}
      {res !== null ? (
        <ResultBox
          label="Body fat"
          ...
          action={
            <SaveButton
              onClick={() =>
                onCommit(
                  `Body fat ${sex} waist ${waist} neck ${neck}`,
        ...
        <ResultBox
          ...
          action={
            <SaveButton
              onClick={() =>
                onCommit(
                  `Water ${weight}kg + ${exercise}min`,
                  `${Math.round(res * 100) / 100} L`,
                  "Health",
                )
              }
            />
